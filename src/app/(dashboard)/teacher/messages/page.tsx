...
export default function TeacherMessagesPage() {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col space-y-4">
      <div>
        <h1 className="text-[17px] font-semibold text-gray-900">Communication Hub</h1>
        <p className="text-[13px] text-gray-500 mt-0.5">
          Stay in touch with your supervised students and company partners.
        </p>
      </div>
      ...
      <div className="flex flex-1 gap-4 overflow-hidden">
        ...
        {internships.length > 1 && (
          <div className="w-56 bg-white border border-gray-200 rounded-md shadow-sm flex flex-col overflow-y-auto flex-shrink-0">
            <p className="px-4 py-3 text-[11px] font-bold text-gray-400 uppercase tracking-wider border-b">
              Threads
            </p>
            {internships.map((i) => (
              <button
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden flex flex-col">
          <div className="h-14 px-6 border-b border-gray-100 flex items-center bg-gray-50/50 gap-3">
            <div className="h-2 w-2 rounded-full bg-green-500" />
            <span className="text-[13px] font-bold text-gray-700 uppercase tracking-tight">
              {selectedInternship ? selectedInternship.topic.title : "No active internship"}
            </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {isLoading ? (
              ...
              <p className="text-center text-gray-400 text-[13px] py-12">No active internships to supervise.</p>
            ) : messages.length === 0 ? (
          ...
          <form onSubmit={handleSend} className="p-4 border-t border-gray-100 bg-gray-50/30">
            <div className="max-w-4xl mx-auto flex items-center gap-2">
              ...
              <input
                type="text"
                placeholder={selectedId ? "Type your message..." : "No active internship"}
