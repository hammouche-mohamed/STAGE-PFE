  ...
  return (
    <div className="space-y-6">
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isRTL ? "sm:flex-row-reverse text-right" : ""}`}>
        <div>
          <h1 className="text-[20px] font-bold text-gray-900">{t("nav.notifications" as any)}</h1>
          <p className="text-[13px] text-gray-500 mt-1">Manage your platform alerts and updates.</p>
        </div>
          ...
          notifications.map((item) => (
            ...
            >
              <div className={`flex items-start gap-4 ${isRTL ? "flex-row-reverse" : ""}`}>
                <div className={`mt-1 ${item.isRead ? "text-gray-300" : "text-indigo-600"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className={`flex items-center justify-between gap-4 mb-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                    <h2 className={`text-sm font-bold ${item.isRead ? "text-gray-600" : "text-gray-900"}`}>
                      {item.title}
                    </h2>
                  ...
                  <p className={`text-[13px] leading-relaxed ${item.isRead ? "text-gray-500" : "text-gray-700"}`}>
                    {item.message}
                  </p>
                  
                  {item.relatedType && (
                    <div className={`mt-3 flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-500 text-[10px] rounded uppercase font-bold tracking-tight">
                        {item.relatedType}
                      </span>
                    </div>
                  )}
