...
function AdminMessagesContent() {
  ...
  // Fetch admin's internships (as thread list)
  ...
  return (
    <div className="-mt-4 md:-mt-6 h-[calc(100vh-115px)] flex flex-col space-y-3 overflow-hidden">
      <div className="flex-shrink-0">
        <h1 className="text-[17px] font-bold text-gray-900 dark:text-white leading-none">Oversight & Messages</h1>
        <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-1">
          Monitor and manage communication threads across active internships.
        </p>
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              <div className="p-4 space-y-3">
                {[1, 2, 3].map(n => (
                  <div key={n} className="h-12 bg-gray-50 dark:bg-slate-800/50 rounded-lg animate-pulse" />
                ))}
              </div>
            ) : internships.length === 0 ? (
              <p className="p-6 text-center text-[12px] text-gray-400 dark:text-gray-500 italic">No active internships</p>
            ) : (
              internships.map((i) => (
                <button
                  key={i.id}
              ...
              <div className="px-4 md:px-6 py-4 border-b border-gray-100 dark:border-slate-800 bg-gray-50/60 dark:bg-slate-800/40 flex flex-col gap-6">
                <div className={`flex items-center justify-between ${isRTL ? "flex-row-reverse" : ""}`}>
                  <div className={`flex items-center gap-2 md:gap-3 ${isRTL ? "flex-row-reverse" : ""}`}>
                    <button onClick={() => setSelectedId(null)} className="md:hidden p-1 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-lg text-gray-500">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full h-8 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-full ${isRTL ? "pr-8 pl-4 text-right" : "pl-8 pr-4"} text-[12px] focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 dark:text-white`}
                  />
                  {searchQuery && (
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="rounded-xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-800 p-2.5 shadow-sm">
                              <div className={`flex items-center gap-4 ${isRTL ? "flex-row-reverse text-right" : ""}`}>
                                <div className="h-8 w-8 rounded-full bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 flex items-center justify-center font-bold text-[12px] flex-shrink-0">
                            ...
                            <div className="space-y-2">
                              {selectedInternship?.students.map((s, idx) => (
                                <div key={`${s.student.email}-${idx}`} className="rounded-xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-800 p-2.5 shadow-sm">
                                  <div className={`flex items-center gap-4 ${isRTL ? "flex-row-reverse text-right" : ""}`}>
                                    <div className="h-8 w-8 rounded-full bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-bold text-[12px] flex-shrink-0">
                            ...
                            sharedFiles.map((file) => (
                              ...
                              >
                                <div className={`h-9 w-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                                  file.category === "DOCUMENT" ? "bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400" : "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400"
                                }`}>
                                  <FileText className="h-4 w-4" />
                                </div>
                                <div className={`min-w-0 flex-1 ${isRTL ? "text-right" : "text-left"}`}>
                                  <p className="text-[12px] font-semibold text-gray-800 dark:text-gray-200 group-hover:text-indigo-700 dark:group-hover:text-indigo-400 transition-colors" title={file.name}>
                                    {file.name}
                                  </p>
