...
const nextConfig: NextConfig = {
  ...
  compress: true,
  // NFR-S3: don't leak server identity.
  ...
  // NFR-S3: security headers. HSTS forces HTTPS for one year on browsers that
  // have already loaded the site over HTTPS (the platform — Vercel — handles
  // the initial HTTP→HTTPS redirect).
  async headers() {
    return [
      {
        source: "/:path*",
